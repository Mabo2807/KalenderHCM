      ...
      try {
        ...
        if (this._selectedDate && dDate) {
          const mid = this._memberIdByDate.get(this._selectedDate)
                   || this._selectedDate.replace(/-/g, ''); // Fallback YYYYMMDD
          filters[dDate] = [mid];
        }
        if (this._selectedStatuses.size && dStat) {
          filters[dStat] = [...this._selectedStatuses].map(s => this._memberIdByStatus.get(s) || s);
        }
        if (this._selectedSchichten.size && dSch) {
          filters[dSch] = [...this._selectedSchichten].map(s => this._memberIdBySchicht.get(s) || s);
        }
