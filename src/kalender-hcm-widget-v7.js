...
(function () {
  ...
  class KalenderHcm extends HTMLElement {
    ...
    _applyLinkedFilters() {
      try {
        ...
        if (this._selectedDate && dDate) {
          filters[dDate] = [this._selectedDate.replace(/-/g, '')]; // YYYYMMDD
        }
        if (this._selectedStatuses.size && dStat) {
          filters[dStat] = [...this._selectedStatuses];
        }
        if (this._selectedSchichten.size && dSch) {
          filters[dSch] = [...this._selectedSchichten];
        }
    ...
    _render() {
      const cssUrl        = _WIDGET_BASE_URL + 'kalender-hcm-v7.css';
