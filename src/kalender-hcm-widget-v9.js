...
(function () {
  ...
  class KalenderHcm extends HTMLElement {
    constructor() {
      ...
      // Echte SAC-Member-IDs aus den Daten (Anzeigewert → Member-ID für setFilters)
      this._memberIdByDate    = new Map();
      this._memberIdByStatus  = new Map();
      this._memberIdBySchicht = new Map();
    }
    ...
    // ── SAC Linked Analysis (Cross-Widget-Filter) ──────────────────────────────

    /** Liefert das echte DataBinding-Objekt (mit getLinkedAnalysis). */
    _getBinding() {
    ...
    /** Feed-ID (z.B. "dateColumn") → echte Dimension-ID (z.B. "Datum").
     *  metadata.feeds[feedId].values[0] = "dateColumn_0"
     *  metadata.dimensions["dateColumn_0"].id = "Datum" */
    _dimId(feedId) {
      const md = this.dataBinding && this.dataBinding.metadata;
      if (!md || !md.feeds || !md.dimensions) return null;
      const feed = md.feeds[feedId];
      const key  = feed && feed.values && feed.values[0];
      const dim  = key && md.dimensions[key];
      return (dim && dim.id) || null;
    }

    /** Wendet alle aktiven Selektionen (Datum + Status + Schicht) als
     *  Linked-Analysis-Filter auf die ganze Seite an. Ein kombinierter Aufruf. */
    _applyLinkedFilters() {
      try {
        const binding = this._getBinding();
        if (!binding || typeof binding.getLinkedAnalysis !== 'function') {
          console.warn('HCM: getLinkedAnalysis nicht verfügbar');
          return;
        }
        const la = binding.getLinkedAnalysis();
        if (!la || typeof la.setFilters !== 'function') {
          console.warn('HCM: LinkedAnalysis.setFilters nicht verfügbar');
          return;
        }

        const filters = {};
        const dDate = this._dimId('dateColumn');
        const dStat = this._dimId('statusColumn');
        const dSch  = this._dimId('schichtColumn');

        // Einzelwert (String) bei genau 1 Member, sonst Array — wie SAP-Beispiel
        const one = (arr) => arr.length === 1 ? arr[0] : arr;

        if (this._selectedDate && dDate) {
          const mid = this._memberIdByDate.get(this._selectedDate)
                   || this._selectedDate; // Fallback ISO mit Bindestrichen
          filters[dDate] = mid;
        }
        if (this._selectedStatuses.size && dStat) {
          filters[dStat] = one([...this._selectedStatuses].map(s => this._memberIdByStatus.get(s) || s));
        }
        if (this._selectedSchichten.size && dSch) {
          filters[dSch] = one([...this._selectedSchichten].map(s => this._memberIdBySchicht.get(s) || s));
        }

        if (Object.keys(filters).length === 0) {
          if (typeof la.removeFilters === 'function') {
            la.removeFilters();
            console.log('HCM LinkedAnalysis: Filter entfernt');
          }
        } else {
          console.log('HCM LinkedAnalysis setFilters:', JSON.stringify(filters));
          la.setFilters(filters);
        }
      } catch (e) {
        console.warn('HCM applyLinkedFilters Fehler:', e.message);
      }
    }

    /** Kompatibilitäts-Wrapper: Argumente werden ignoriert, es wird immer der
     *  komplette Selektionszustand angewendet. */
    _trySetMemberFilter() {
      this._applyLinkedFilters();
    }
    ...
    _processDataBinding() {
      ...
      try {
        ...
        // Echte Member-ID einer Zelle (für setFilters)
        const rawId = (row, key) => {
          const v = row[key];
          if (v === undefined || v === null) return '';
          return (typeof v === 'object') ? (v.id != null ? String(v.id) : '') : String(v);
        };
        ...
        // Einmalig erste Zeile loggen, um echtes Member-ID-Format zu sehen
        if (!this._rowLogged) {
          this._rowLogged = true;
          try {
            console.log('HCM ROW0:', JSON.stringify(firstRow).substring(0, 800));
            console.log('HCM keys:', JSON.stringify({ dateFeedId, statusFeedId, schichtFeedId }));
          } catch (e) {}
        }
        ...
        for (const row of rows) {
          ...
          if (date) {
            const dId = rawId(row, dateFeedId);
            if (dId) this._memberIdByDate.set(date, dId);
          }
          if (date && status) {
            this._statusMap.set(date, status);
            const sId = rawId(row, statusFeedId);
            if (sId) this._memberIdByStatus.set(status, sId);
          }
          if (date && schicht) {
            this._schichtMap.set(date, schicht);
            const scId = rawId(row, schichtFeedId);
            if (scId) this._memberIdBySchicht.set(schicht, scId);
          }
    ...
    _render() {
      const cssUrl        = _WIDGET_BASE_URL + 'kalender-hcm-v9.css';
