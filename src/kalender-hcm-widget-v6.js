...
(function () {
  ...
  class KalenderHcm extends HTMLElement {
    ...
    /** Feed-ID (z.B. "dateColumn") → echte Dimension-ID. Mehrere Wege. */
    _resolveDimensionId(feedId, binding) {
      // Weg 1: Metadata-Feed (verschiedene mögliche Strukturen)
      const db   = this.dataBinding;
      const feed = db && db.metadata && db.metadata.feeds && db.metadata.feeds[feedId];
      if (feed) {
        if (feed.values && feed.values[0] && feed.values[0].id) return feed.values[0].id;
        if (feed.id) return feed.id;
        if (Array.isArray(feed) && feed[0] && feed[0].id) return feed[0].id;
      }
      // Weg 2: binding.getDimensions(feedId)
      try {
        if (binding && typeof binding.getDimensions === 'function') {
          const dims = binding.getDimensions(feedId);
          if (Array.isArray(dims) && dims[0]) return dims[0].id || dims[0].name || null;
          if (dims && dims.id) return dims.id;
        }
      } catch (e) { console.log('HCM getDimensions error:', e.message); }
      return null;
    }

    /** Einmaliger Komplett-Dump zur Struktur-Diagnose. */
    _diagOnce(binding) {
      if (this._diagDumped) return;
      this._diagDumped = true;
      try {
        const md = this.dataBinding && this.dataBinding.metadata;
        console.log('HCM METADATA:', JSON.stringify(md).substring(0, 1500));
      } catch (e) { console.log('HCM metadata dump error:', e.message); }
      try {
        if (binding && typeof binding.getDimensions === 'function') {
          const all = binding.getDimensions();
          console.log('HCM binding.getDimensions():', JSON.stringify(all).substring(0, 1000));
          ['dateColumn','statusColumn','schichtColumn'].forEach(f => {
            try { console.log('HCM getDimensions("'+f+'"):', JSON.stringify(binding.getDimensions(f)).substring(0,500)); }
            catch(e) { console.log('HCM getDimensions("'+f+'") error:', e.message); }
          });
        }
      } catch (e) { console.log('HCM getDimensions dump error:', e.message); }
      try {
        if (binding && typeof binding.getLinkedAnalysis === 'function') {
          this._dumpObj('LinkedAnalysis', binding.getLinkedAnalysis());
        }
    ...
    _trySetMemberFilter(feedId, memberKeys) {
      try {
        ...
        // Einmalige Struktur-Diagnose
        this._diagOnce(binding);

        const dimId = this._resolveDimensionId(feedId, binding);
        if (!dimId) { console.warn('HCM: keine Dimension-ID für', feedId); return; }
        console.log('HCM Dimension-ID für', feedId, '=', dimId);
    ...
    _render() {
      const cssUrl        = _WIDGET_BASE_URL + 'kalender-hcm-v6.css';
